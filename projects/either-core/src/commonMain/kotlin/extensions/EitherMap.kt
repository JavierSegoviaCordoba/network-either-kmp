...
public inline fun <L, R, NL, NR> Either<L, R>.map(
    left: (L) -> NL,
    right: (R) -> NR,
): Either<NL, NR> =
    when (this) {
...
public inline fun <L, R, NL> Either<L, R>.mapLeft(left: (L) -> NL): Either<NL, R> =
    when (this) {
        is Either.Left -> Either.Left(left(this.left))
...
public inline fun <L, R, NR> Either<L, R>.mapRight(right: (R) -> NR): Either<L, NR> =
    when (this) {
        is Either.Left -> this
        is Either.Right -> Either.Right(right(this.right))
    }
