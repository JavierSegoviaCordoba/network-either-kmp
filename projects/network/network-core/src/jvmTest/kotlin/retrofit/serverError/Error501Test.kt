...
internal class Error501Test : BaseTest<ErrorDTO, DogDTO>() {
    ...
    override val codeToFile: Pair<Int, String?> = 501 to "5xx.json"
    override val expected: NetworkEither<ErrorDTO, DogDTO> =
        buildNetworkFailureHttp(error, code, headers)

    @Test fun `suspend call 501`() = runTestBlocking { service.getDog() shouldBe expected }

    @Test
    fun `async call 501`() = runTestBlocking { service.getDogAsync().await() shouldBe expected }
}
