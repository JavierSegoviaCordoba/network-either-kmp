...
internal class Error410Test : BaseTest<ErrorDTO, DogDTO>() {
    ...
    override val codeToFile: Pair<Int, String?> = 410 to "4xx.json"
    override val expected: NetworkEither<ErrorDTO, DogDTO> =
        buildNetworkFailureHttp(error, code, headers)

    @Test fun `suspend call 410`() = runTestBlocking { service.getDog() shouldBe expected }

    @Test
    fun `async call 410`() = runTestBlocking { service.getDogAsync().await() shouldBe expected }
}
