...
internal class Error411Test : BaseTest<ErrorDTO, DogDTO>() {
    ...
    override val codeToFile: Pair<Int, String?> = 411 to "4xx.json"
    override val expected: NetworkEither<ErrorDTO, DogDTO> =
        buildNetworkFailureHttp(error, code, headers)

    @Test fun `suspend call 411`() = runTestBlocking { service.getDog() shouldBe expected }

    @Test
    fun `async call 411`() = runTestBlocking { service.getDogAsync().await() shouldBe expected }
}
