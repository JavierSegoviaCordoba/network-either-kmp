        ...
        when (code) {
            in 200..299 -> {
                if (body != null)
                    onResponse(
                        call,
                        Response.success(buildNetworkSuccess(body, HttpStatusCode(code), headers))
                    )
            ...
            in 400..599 -> {
                if (errorBody != null)
                    onResponse(
                        call,
                        Response.success(
                            buildNetworkFailureHttp(errorBody, HttpStatusCode(code), headers)
                        )
                    )
...
private fun <F : Any, S : Any> handleNullBody(
    ...
    try {
        callback.onResponse(
            call,
            Response.success(buildNetworkSuccess(Unit as S, HttpStatusCode(code), headers))
        )
