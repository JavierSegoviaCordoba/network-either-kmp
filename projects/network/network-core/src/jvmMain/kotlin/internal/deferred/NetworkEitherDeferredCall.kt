...
private fun <F, S> onEOFException(deferred: CompletableDeferred<NetworkEither<F, S>>) {
    ...
    try {
        deferred.complete(buildNetworkSuccess(Unit as S, HttpStatusCode.NoContent.value, emptyHeader.toMap()))
...
private fun <F, S> onIllegalStateException(deferred: CompletableDeferred<NetworkEither<F, S>>, throwable: Throwable) {
    printlnError(
        """
           | # # # # # # # # # # # # # # ERROR # # # # # # # # # # # # # # #
...
private fun <F, S> onCommonConnectionException(
    ...
    deferred.complete(if (isNetworkAvailable) buildNetworkFailureRemote() else buildNetworkFailureLocal())
}
