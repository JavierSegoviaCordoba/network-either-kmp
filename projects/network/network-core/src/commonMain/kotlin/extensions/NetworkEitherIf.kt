...
public fun <F, S> NetworkEither<F, S>.ifFailureHttp(block: (F, HttpStatusCode, Headers) -> Unit) {
...
public fun <F, S> NetworkEither<F, S>.ifFailureHttpOnlyCode(block: (HttpStatusCode) -> Unit) {
    if (this is NetworkFailure<F> && left is Failure.Http<F>) {
...
public fun <F, S> NetworkEither<F, S>.ifSuccess(block: (S, HttpStatusCode, Headers) -> Unit) {
