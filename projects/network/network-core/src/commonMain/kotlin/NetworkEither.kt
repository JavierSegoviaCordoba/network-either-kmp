...
public sealed class Failure<out F> {
    ...
    public data class Http<F>(val error: F, val code: Int, val headers: Headers) : Failure<F>()
...
public data class Success<S>(val data: S, val code: Int, val headers: Headers)

public typealias Headers = Map<String, List<String>>

public fun <F, S> buildNetworkFailureHttp(error: F, code: Int, headers: Headers): NetworkEither<F, S> =
    Either.Left(Failure.Http(error = error, code = code, headers = headers))
...
public fun <F, S> buildNetworkSuccess(data: S, code: Int, headers: Headers): NetworkEither<F, S> =
    Either.Right(Success(data = data, code = code, headers = headers))
