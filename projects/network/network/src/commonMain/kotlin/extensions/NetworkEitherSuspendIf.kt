...
@JvmName("suspendIfFailureHttpError")
public suspend fun <F, S> NetworkEither<F, S>.suspendIfFailureHttp(block: suspend (F) -> Unit) {
    if (this is NetworkFailure<F> && left is Failure.Http<F>) {
        with(left as Failure.Http<F>) { block(error) }
    }
}

@JvmName("suspendIfFailureHttpCode")
public suspend fun <F, S> NetworkEither<F, S>.suspendIfFailureHttp(block: suspend (Int) -> Unit) {
    if (this is NetworkFailure<F> && left is Failure.Http<F>) {
...
@JvmName("suspendIfSuccessData")
public suspend fun <F, S> NetworkEither<F, S>.suspendIfSuccess(block: suspend (S) -> Unit) {
