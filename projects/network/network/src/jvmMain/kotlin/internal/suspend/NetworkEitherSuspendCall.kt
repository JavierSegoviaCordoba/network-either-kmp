...
internal class NetworkEitherSuspendCall<F : Any, S : Any>(
    ...
    override fun enqueue(callback: Callback<NetworkEither<F, S>>) = synchronized(this) {
        backingCall.enqueue(
            object : Callback<S> {
...
private fun <F, S> Call<NetworkEither<F, S>>.onEOFException(callback: Callback<NetworkEither<F, S>>) {
    printlnWarning(
        """
           | # # # # # # # # # # # # # # WARNING # # # # # # # # # # # # # # # # # # #
    ...
    try {
        callback.onResponse(
            this,
            Response.success(buildNetworkSuccess(Unit as S, HttpStatusCode.NoContent.value, emptyHeader.toMap()))
        )
